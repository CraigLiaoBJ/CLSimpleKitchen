...
#import "CLNewHeadReusableView.h"
#import "CLSubMenuViewCell.h"

@interface CLNewDetailViewController ()<UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>{
...
@implementation CLNewDetailViewController
...
- (void)setupCollectionView{
    ...
    [self.collectionView registerClass:[CLNewHeadReusableView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"HEADCELL"];
}

#pragma mark --
- (void)loadData{
    NSString *string = [NewDetail stringByAppendingFormat:@"%@&is_traditional=0&phonetype=1", self.NewDetailId];
    ...
    [manager GET:string parameters:nil success:^(AFHTTPRequestOperation * operation, id responseObject) {
        ...
        for (NSDictionary *tempDic in array) {
            ...
            NSArray *vArray = tempDic[@"vegetable"];
            for (NSDictionary *dic in vArray) {
                CLModelItem *item = [[CLModelItem alloc] init];
...
{
    if (self.isShow) {
        return CGSizeMake(0, _height + 240);
    } else {
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath{
    CLNewHeadReusableView *headView = [collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:@"HEADCELL" forIndexPath:indexPath];
    //赋值
    headView.headItem = modelItem;
    
    //计算高度
    _height= [self getHeightWithString:modelItem.content font:13 width:kWIDTH-20];
